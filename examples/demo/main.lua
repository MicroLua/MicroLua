...
local function demo_sysinfo()
    io.printf("Chip: %d, ROM: %d, core: %d, SDK: %s, Lua: %s\n",
...
local function demo_uart()
    ...
    thread.start(function()
        io.printf("UART: Tx: start\n")
        u:write_blocking(data)
        u:tx_wait_blocking()
        io.printf("UART: Tx: done\n")
...
function main()
    ...
    io.printf(string.rep("=", 79) .. "\n")
    io.printf("Main thread [%s], startup time: %s us\n",
